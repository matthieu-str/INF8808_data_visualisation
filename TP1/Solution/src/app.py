...
def generate_data():
    ...
    '''
    nb_of_pts = np.random.randint(1, 11)
    table = np.random.randint(1, 99, size=(nb_of_pts, 2))
    return pd.DataFrame(table, columns=['x', 'y'])
...
def init_figure(dataframe, layout):
    '''
        ...
        The figure's data is initialised as a Scatter object. The mode is "markers" and
...
def init_app_layout(figure):
    ...
    return html.Div(children=[
        ...
        html.Header(children=[
            html.H1(children='TP1'),
            html.Div(
                children='Bienvenue au cours INF8808 : Visualisation de données.'
            )
        ]),
...
def update_label(figure):
    ...
    nb_of_pts = len(figure.data[0].x)

    label_elements = [
        html.Span('Il y a : '),
        html.Span(nb_of_pts, style={'fontWeight': 'bold'}),
        html.Span(' points' if nb_of_pts > 1 else ' point')
    ]
